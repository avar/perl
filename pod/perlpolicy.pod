...
A change that is cherry-picked to maint should have a signed-off-by
entry for each commiter that agrees to its integration. This can be
done as:

    # cherry-pick a commit
    git checkout maint-5.12
    git cherry-pick COMMIT

    # Edit the commit in an editor. Add any missing Signed-off-by
    # lines manually for missing commiters that agreed
